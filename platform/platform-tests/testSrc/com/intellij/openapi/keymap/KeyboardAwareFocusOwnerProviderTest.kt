...
internal class KeyboardAwareFocusOwnerProviderTest {
  ...
  /**
   * The ancestor chain is resolved once per focus owner, not per key event: this hatch sits on the IDE's key-dispatch
   * path, so a walk to the root on every keystroke would be a cost paid by every user, including those with no
   * embedded renderer anywhere. Caching must not change what is asked, only how often it is looked up — the provider
   * is still consulted for every event, because whether it wants one is per-event state.
   */
  @Test
  fun `provider is consulted for every event even though the chain is resolved once`() {
    val inner = InnerComponent()
    val panel = ProviderPanel()
    panel.add(inner)
    KeyboardFocusManager.setCurrentKeyboardFocusManager(StubFocusManager(inner))
    ...
    assertEquals(3, panel.consultedEvents.size, "every event reaches the provider")
  }

  @Test
  fun `a different focus owner re-resolves the chain instead of reusing the previous one`() {
    val firstInner = InnerComponent()
    val firstPanel = ProviderPanel()
    firstPanel.add(firstInner)
    firstPanel.claim = { true }

    // A second focus owner with no provider above it: reusing the first owner's chain would wrongly claim here.
    val bareInner = InnerComponent()
    JPanel().add(bareInner)

    val dispatcher = IdeKeyEventDispatcher(null)
    KeyboardFocusManager.setCurrentKeyboardFocusManager(StubFocusManager(firstInner))
    assertFalse(dispatcher.dispatchKeyEvent(pressed(firstInner, SINGLE_STROKE)))
    assertEquals(0, singleAction.invocations.get(), "claimed for the first owner")

    KeyboardFocusManager.setCurrentKeyboardFocusManager(StubFocusManager(bareInner))
    dispatcher.dispatchKeyEvent(pressed(bareInner, SINGLE_STROKE))
    assertEquals(1, singleAction.invocations.get(), "the provider-less owner is dispatched normally")
  }

  @Test
  fun `a chain rebuilt without a focus change is re-resolved`() {
    // Reparenting does not move focus, so a cache keyed only on the focus owner would keep answering with a
    // provider that is no longer an ancestor.
    val inner = InnerComponent()
    val panel = ProviderPanel()
    panel.add(inner)
    panel.claim = { true }
    ...
    // Same focus owner, no longer under the provider.
    JPanel().add(inner)
    dispatcher.dispatchKeyEvent(pressed(inner, SINGLE_STROKE))
    assertEquals(1, singleAction.invocations.get(), "the stale provider must not still claim")
  }
