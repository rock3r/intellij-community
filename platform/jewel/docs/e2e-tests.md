...
Linux under Xvfb, and one on Windows. Each job runs the IDE test in its own step, selected by its `jewel-ide-e2e` tag.
The IDE test's build compiles the whole platform, and the standalone tests time out when they run during that build.
