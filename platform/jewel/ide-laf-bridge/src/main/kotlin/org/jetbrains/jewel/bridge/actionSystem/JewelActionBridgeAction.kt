...
public open class JewelActionBridgeAction : AnAction() {
    ...
    override fun update(event: AnActionEvent) {
        ...
        // Only override the platform icon when Jewel actually resolved one: a null sample must leave the
        // action's declared (template) icon standing, which is the fallback rule ActionButton applies too.
        swingIconFor(presentation.icon)?.let { event.presentation.icon = it }
        // Presentation properties are IJPL client properties by another name, and both sides key them by
        // name (Presentation stores its user map by Key.toString()), so they round-trip verbatim — including
        // toggle state, which is Toggleable.SELECTED_KEY on one side and ActionPropertyNames.Selected on the
        // other. Nothing here needs to know which keys exist.
        for ((name, value) in presentation.properties) {
            event.presentation.putClientProperty(Key.create<Any>(name), value)
        }
        // Popup retention has a typed platform equivalent (the 4-state KeepPopupOnPerform), so it is mapped.
        presentation.menuDismissPolicy?.let { event.presentation.keepPopupOnPerform = it.toPlatform() }
    }

    /**
     * Converting an icon descriptor to a Swing icon loads image resources, and `update()` runs on the platform's
     * cadence (up to twice a second per place), so the conversion is memoised on the descriptor's value identity.
     * Benign races only overwrite the cache with an equivalent conversion.
     */
    @Volatile private var cachedIconSource: com.intellij.platform.icons.Icon? = null

    @Volatile private var cachedSwingIcon: javax.swing.Icon? = null

    private fun swingIconFor(icon: com.intellij.platform.icons.Icon?): javax.swing.Icon? {
        if (icon == null) return null
        if (icon == cachedIconSource) return cachedSwingIcon
        val converted = icon.toSwingIcon()
        cachedIconSource = icon
        cachedSwingIcon = converted
        return converted
    }
 ...
 */
// The annotation must target the getter: on a top-level property, a bare @ApiStatus.Internal does not
// reach the generated file-facade method, and the key would surface in the stable API dump.
