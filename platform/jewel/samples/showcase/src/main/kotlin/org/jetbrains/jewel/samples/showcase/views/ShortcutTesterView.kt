 ...
 * and (when a mutable keymap is passed) live rebinding.
 *
 ...
 */
@Composable
public fun ShortcutTesterView(
    modifier: Modifier = Modifier,
    keymap: MutableJewelKeymap? = null,
    ...
    Column(
        ...
        // ----- 5. Action-bound components agree with the keyboard -----
        GroupHeader("Action components")
        Row(Modifier.testCard(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ActionButton(DemoSelectAll)
            ActionButton(DemoPing)
            Text("Buttons sample the same presentation and invoke through the same host as the keyboard.")
        }

        // ----- 6. Live rebinding -----
        if (keymap != null && registry != null) {
            GroupHeader("Keymap — rebind live, no recomposition of the bindings needed")
            KeymapSettingsPanel(keymap = keymap, registry = registry, modifier = Modifier.testCard())
        }
    }
}
